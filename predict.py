...
if __name__ == '__main__': 
    ...
    print(f"mAP@0.5: {metrics.box.map50:.3f}")  # Mean AP at IoU=0.5
    print(f"mAP@0.5-0.95: {metrics.box.map:.3f}")  # Mean AP across IoUs
    print(f"Precision: {sum(metrics.box.p)/len(metrics.box.p):.3f}")  # Mean precision
    print(f"Recall: {sum(metrics.box.r)/len(metrics.box.r):.3f}")  # Mean recall
    print(f"F1 Score: {sum(metrics.box.f1)/len(metrics.box.f1):.3f}")